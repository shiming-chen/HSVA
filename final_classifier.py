...
class CLASSIFIER:
    def __init__(self, model, _train_X, _train_Y,_test_seen_X,_test_seen_Y,_test_unseen_X, _test_unseen_Y, seenclasses,unseenclasses,
                 _nclass, device , selected_cls, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100,  generalized=True, use = None, ignore = None,train_only=False,test_only=False,do_nothing=False):
        self.selected_cls = selected_cls
        
        self.train_only = train_only
        self.device = device

        if selected_cls == 'softmax':
            ...
            self.test_unseen_feature = _test_unseen_X.to(self.device)
            self.test_unseen_label = _test_unseen_Y.to(self.device)


            self.seenclasses = seenclasses.to(self.device)
            self.unseenclasses = unseenclasses.to(self.device)
        if selected_cls == 'knn':
            self.train_X =  _train_X.cpu() 
            self.train_Y = _train_Y.cpu() 

            self.test_seen_feature = _test_seen_X.cpu() 
            self.test_seen_label = _test_seen_Y.cpu() 
            self.test_unseen_feature = _test_unseen_X.cpu() 
            self.test_unseen_label = _test_unseen_Y.cpu() 


            self.seenclasses = seenclasses.cpu() 
            self.unseenclasses = unseenclasses.cpu() 
        ...
        self.acc_unseen, self.acc_seen, self.H , self.acc = 0, 0, 0, 0
            ...
            if test_only==False:
                if generalized:
                    if selected_cls == 'softmax':
                        ...
                        self.acc_seen = self.compute_per_class_acc_gzsl_knn( self.test_seen_label, pred_Y_s, self.seenclasses)
                        self.acc_unseen = self.compute_per_class_acc_gzsl_knn( self.test_unseen_label, pred_Y_u,  self.unseenclasses)
                ...
                if generalized:
                    ...
                    if H > best_H:
                        ...
                        best_H = H
                    self.acc_seen, self.acc_unseen, self.H =best_seen,best_unseen,best_H
    ...
    def compute_per_class_acc_gzsl_knn( self, test_label, predicted_label, target_classes):
        acc_per_class = 0
